...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    device: torch.device, epoch: int, max_norm: float = 0, dataset_file: str = "", use_auxiliary_text: bool = False):
    model.train()
    criterion.train()
    ...
    for images, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if use_auxiliary_text:
            outputs = model(images.tensors, texts, images.mask, auxiliary_texts)
        else:
...
def evaluate(model, postprocessors, criterion, data_loader, device, args):
    ...
    text_features, auxiliary_text_features = prepare_text_inputs(model, data_loader.dataset.dataset_texts, device, hoi_descriptions)
...
def prepare_text_inputs(model, texts, device, hoi_descriptions):
    ...
    return text_features, auxiliary_text_features
